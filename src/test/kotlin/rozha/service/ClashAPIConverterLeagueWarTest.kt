...
class ClashAPIConverterLeagueWarTest : Spek({
    describe("convert clan war responses") {
        ...
        it("response should have valid values") {
            val testContent = File(Paths.get("").toAbsolutePath().toString() +
                    "/src/test/resources/rozha/leagueWarInWar.json").readText()
            val testee = Parser.default().parse(StringBuilder(clashAPIConverter.convertLeagueWar(testContent)))
